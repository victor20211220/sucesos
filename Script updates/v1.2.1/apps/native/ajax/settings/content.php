...
if (empty($cl["is_logged"])) {
    ...
    if (empty($email)) {
        ...
        if (cl_send_mail($send_email_data)) {
            cl_session("email_conf_code", $rand_code);
            cl_session("new_email", cl_text_secure($email));
    ...
    $data['err_code'] = 0;
    $data['status']   = 400;
    $email_conf_code1 = cl_session('email_conf_code');
    $email_conf_code2 = fetch_or_get($_POST['code'], false);
    $new_email        = cl_session('new_email');
