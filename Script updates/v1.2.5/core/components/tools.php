...
function cl_linkify_emails($text = "") {
    if (empty($text)) {
        return $text;
    }
        ...
        try {
            $text = preg_replace_callback('/(?P<email>([\w\.\-]+)@([\w\-]+)((\.(\w){2,3})+))/is', function($m) {
                if (isset($m['email'])) {

                    $email = cl_add_http_scheme($m['email']);

                    return cl_html_el('a', cl_croptxt($m['email'], 26, "..."), array(
                        'href' => cl_strf("mailto:%s", $email),
                        'target' => '_blank',
                        'class' => 'inline-link'
                    ));
                }
